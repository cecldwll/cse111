...
def print_death_age(people_dict):
    ...
    for key, value in people_dict.items():
        ...
        dob = value[BIRTH_YEAR_INDEX]
        dod = value[DEATH_YEAR_INDEX]
...
def count_genders(people_dict):
    ...
    print(f'Number of males: {total_male}')
    print(f'Number of females: {total_female}')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_key, marriage_list in marriages_dict.items():
        ...
        # Get the husband person key, wife person key, and year they were married.
        husband = marriage_list[HUSBAND_KEY_INDEX]
        wife = marriage_list[WIFE_KEY_INDEX]
        wedding_year = marriage_list[WEDDING_YEAR_INDEX]

        # Use the husband person key to get the husband's data from the people dictionary and then get the
        # husband's name and birth year from his data.
        hlist = people_dict[husband]
        hname = hlist[NAME_INDEX]
        hdob = hlist[BIRTH_YEAR_INDEX]

        # Calculate the husband's age when he married.
        hage = wedding_year - hdob

        # Use the wife person key to get the wife's data from the people dictionary and then get the
        # wife's name and birth year from her data.
        wlist = people_dict[wife]
        wname = wlist[NAME_INDEX]
        wdob = wlist[BIRTH_YEAR_INDEX]

        # Calculate the wife's age when she married.
        wage = wedding_year - wdob

        # Print the data about the marriage for the user to see.
        print(f"{hname} {hage} > {wedding_year} < {wname} {wage}")
